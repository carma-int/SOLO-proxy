...
function onrequest(req, res) {
	...
	authenticate(server, req, function(err, auth) {
		...
		if (null != agent) {
			...
			//console.log("setting custom `http.Agent` option for proxy request: " , agent)
			debug.proxyRequest(
		...
		if (parsed.headers['X-Forwarded-For'])   delete parsed.headers['X-Forwarded-For'];
		if (parsed.headers['Via']) delete	parsed.headers['Via'];
